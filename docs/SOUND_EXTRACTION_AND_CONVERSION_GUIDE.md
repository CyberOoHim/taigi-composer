...
本專案致力於台語經典歌曲與簡譜（Numbered Notation / 記譜法）的數位化創作、卡拉OK伴奏、羅馬字歌詞對齊（POJ 白話字）與漢羅（Han-lô）與聲音合成。
...
- **正確處理規則**：
  - 首個起音音符填入歌詞漢羅與羅馬字（`hanlo: "花", poj: "hoe"`）。
...
export interface JianpuNote {
  ...
  lyric: {
    hanlo?: string;        // 漢羅 / 漢字 (例如 "雨", "hōo-miâ")
    poj?: string;          // 羅馬字 / 白話字 (例如 "Hō͘")
  };
