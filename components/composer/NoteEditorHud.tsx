...
export const NoteEditorHud: React.FC<NoteEditorHudProps> = ({
  ...
  return (
    <div
      ...
      className="sticky top-[68px] z-30 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md border-2 border-amber-500 rounded-2xl shadow-xl transition-all duration-200 overflow-hidden"
    >
      {/* Top Bar: Always visible, streamlined for iPad & desktop */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-3.5 py-2.5 bg-amber-50/90 dark:bg-amber-950/40 border-b border-amber-200 dark:border-zinc-800 text-xs">
        {/* Note Status & Audition */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2.5 py-1 bg-amber-500 text-zinc-950 rounded-lg font-black font-mono text-xs shadow-2xs">
            #{selectedMeasureIndex + 1}.{(selectedNoteIndex ?? 0) + 1}
          </span>

          <div className="flex items-center gap-1.5 font-bold text-zinc-900 dark:text-zinc-100">
            <span className="text-amber-700 dark:text-amber-300 font-mono text-sm bg-amber-200/60 dark:bg-amber-900/60 px-2 py-0.5 rounded-md">
              {pitchLabel}
            </span>
          ...
          {currentNote.annotation && (
            <span className="text-[10px] bg-indigo-100 dark:bg-indigo-950 text-indigo-800 dark:text-indigo-200 px-2 py-0.5 rounded-md font-bold border border-indigo-200 dark:border-indigo-800">
              {currentNote.annotation}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 bg-amber-200 hover:bg-amber-300 dark:bg-amber-900 dark:hover:bg-amber-800 text-amber-950 dark:text-amber-100 rounded-xl text-xs font-bold transition-all active:scale-95 cursor-pointer touch-manipulation min-h-[36px]"
            title="試聽單音 (Play Single Note)"
          >
          ...
          {onNavigatePrevNote && onNavigateNextNote && (
            <div className="flex items-center bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 p-0.5">
              <button
                type="button"
          ...
          <button
            ...
            title={isCollapsed ? '展開控制盤 (Expand Control Deck)' : '收合控制盤 (Collapse Deck)'}
          >
            {isCollapsed ? (
              <>
      ...
      {!isCollapsed && (
        <div className="p-3.5 flex flex-col gap-3">
          {/* Deck Mode Selector & Auto-Step Switch */}
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-200/80 dark:border-zinc-800 pb-2.5">
            {/* 3 Modular Tabs */}
            <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl border border-zinc-200/80 dark:border-zinc-700 text-xs font-bold">
              <button
                type="button"
              ...
              >
                ...
                <span>歌詞與註解 (Lyrics/Annotations)</span>
              </button>
            </div>

            {/* Auto-Step Next Note Mode */}
            {onToggleAutoStepAdvance && (
              <button
                type="button"
                onClick={onToggleAutoStepAdvance}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border transition-all cursor-pointer touch-manipulation min-h-[36px] ${
                  autoStepAdvance
          ...
          {activeTab === 'numpad' && (
            <div className="flex flex-col gap-3">
              {/* Pitch Pads (1-7, 0, ␣) with iPad Touch Optimized Size (44px+) */}
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider shrink-0 w-16">
                  音高 1-7:
                </span>
                <div className="flex items-center gap-1.5 flex-wrap flex-1">
                  {[1, 2, 3, 4, 5, 6, 7].map(p => (
                    <button
                      ...
                      className={`flex-1 min-w-[44px] h-11 rounded-xl font-mono text-lg font-black transition-all active:scale-95 cursor-pointer touch-manipulation flex items-center justify-center ${
                        currentNote.pitch === p
                          ? 'bg-amber-500 text-zinc-950 ring-2 ring-amber-400 shadow-md scale-105'
                          : 'bg-zinc-100 dark:bg-zinc-800 hover:bg-amber-100 dark:hover:bg-zinc-700 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700'
                      }`}
                    >
                      {p}
                    </button>
                  ...
                  <button
                    ...
                    className={`min-w-[48px] h-11 rounded-xl font-mono text-base font-bold transition-all active:scale-95 cursor-pointer touch-manipulation flex items-center justify-center gap-1 px-2 ${
                      currentNote.pitch === 0
                        ? 'bg-amber-500 text-zinc-950 ring-2 ring-amber-400 shadow-md font-black'
                        : 'bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700'
                    }`}
                    title="休止符 0"
                  >
                    <span>0</span>
                  ...
                  <button
                    ...
                    className={`min-w-[48px] h-11 rounded-xl font-mono text-xs font-bold transition-all active:scale-95 cursor-pointer touch-manipulation flex items-center justify-center gap-1 px-2 border-dashed ${
                      currentNote.pitch === 'empty'
                        ? 'bg-amber-500 text-zinc-950 ring-2 ring-amber-400 shadow-md font-black border-amber-400'
                        : 'bg-zinc-100/70 dark:bg-zinc-800/70 text-zinc-600 dark:text-zinc-400 border-zinc-300 dark:border-zinc-600'
                    }`}
              ...
              {/* Pitch Modifiers (Octave & Accidental) */}
              <div className="flex flex-wrap items-center gap-4 text-xs">
                ...
                {/* Accidental Controls */}
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider shrink-0">
                    升降音:
                  </span>
                  <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl border border-zinc-200 dark:border-zinc-700">
                    <button
                ...
                <div className="flex items-center gap-1.5 flex-wrap flex-1">
                  {[
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer touch-manipulation min-h-[40px] ${
                        currentNote.duration === d.dur
                          ? 'bg-amber-500 text-zinc-950 font-black shadow-xs ring-2 ring-amber-400'
                          : 'bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-700'
                      }`}
                    >
                      {d.label}
                    </button>
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-xl text-xs font-bold border transition-all cursor-pointer touch-manipulation min-h-[40px] ${
                      currentNote.isDotted
                        ? 'bg-amber-500 text-zinc-950 border-amber-500 ring-2 ring-amber-400'
          ...
          {activeTab === 'lyrics' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-zinc-200 dark:border-zinc-800 text-xs">
                <span className="text-[11px] font-bold text-zinc-500 shrink-0">自訂註解:</span>
                <input
                  type="text"
                  placeholder="如: (副歌前/男聲獨唱/自由節奏)..."
                  value={currentNote.annotation || ''}
                  onChange={e => onSetAnnotation(e.target.value)}
                  className="flex-1 px-3 py-2 text-xs bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-amber-500 text-zinc-900 dark:text-zinc-100"
                />
                <button
                  type="button"
                  onClick={() => {
